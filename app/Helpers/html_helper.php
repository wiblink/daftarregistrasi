...
function btn_action($data = []) {
	...
	foreach ($data as $key => $val) 
	{
		...
		else if ($key == 'return') 
		{
			...
			$html .= '<a href="'.$data[$key]['url'].'" ' . $attr . '>
						<span class="btn-label-icon"><i class="fa fa-share pe-1"></i></span> Return
					</a>';
		}

		else if ($key == 'detil_finish') 
		{
			$btn_class = 'btn btn-success btn-xs me-1';
			if (!key_exists('attr', $val)) {
			...
			$html .= '<a href="'.$data[$key]['url'].'" ' . $attr . '>
						<span class="btn-label-icon"><i class="fa fa-address-book pe-1"></i></span> Detil
					</a>';
		}
