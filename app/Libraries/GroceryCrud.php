...
/**
* PERUBAHAN, tambah method setDataTablesAjaxUrl() dan data_tables_ajax_url, setDataTablesAjaxSetting ditambahkan ke showList
 * PHP grocery CRUD
 *
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
    ...
    protected function showList($ajax = false, $state_info = null)
    {
        ...
        $data->order_by				= $this->order_by;
		$data->data_tables_ajax_url = $this->data_tables_ajax_url;
		$data->data_tables_ajax_setting = $this->data_tables_ajax_setting;
		$data->database_table		= $this->basic_db_table;
    ...
    protected function showEditForm($state_info)
    {
		...
		// echo '<pre>'; print_r($language_alias); die;
		$data->language_alias = $language_alias[strtolower($this->language)];
    ...
    protected function load_select2()
    {
        $this->set_css($this->default_assets_path.'/vendors/jquery.select2/css/select2.min.css');
        $this->set_css($this->default_assets_path.'/vendors/jquery.select2/bootstrap-5-theme/select2-bootstrap-5-theme.min.css');
        $this->set_js_lib($this->default_assets_path.'/vendors/jquery.select2/js/select2.full.min.js');
    }
    ...
    {
        $this->set_css($this->default_assets_path.'/vendors/flatpickr/dist/flatpickr.min.css');
		$this->set_css($this->default_assets_path.'/vendors/flatpickr/dist/themes/' . $_COOKIE['jwd_adm_theme'] . '.css');
        $this->set_js_lib($this->default_assets_path.'/vendors/flatpickr/dist/flatpickr.js');
        ...
        if($this->language !== 'English')
        {
            ...
            { 
		// echo '<pre>';
			// print_r($language_alias); die;
                $i18n_date_js_file = $this->default_assets_path.'/vendors/flatpickr/dist/l10n/'.$language_alias[$this->language].'.js';
                ...
                $i18n_datetime_js_file = $this->default_assets_path.'/vendors/flatpickr/dist/l10n/'.$language_alias[$this->language].'.js';
        ...
        $this->set_js_config($this->default_javascript_path.'/config/flatpickr.config.js');
    ...
    {
        $this->load_select2();
        $this->set_js_config($this->default_javascript_path.'/config/select2.config.js');

        $select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));

        $input = "<select id='field-{$field_info->name}' name='{$field_info->name}' class='chosen-select form-select' data-placeholder='".$select_title."'>";
    ...
    protected function get_relation_input($field_info,$value)
    {
        ...
        $ajax_or_not_class = $using_ajax ? 'chosen-select form-select' : 'chosen-select form-select';
...
class GroceryCrud extends grocery_CRUD_States
{
     ...
     */
    const	VERSION = "2.0.2";
    const	JQUERY 			= "jquery-3.6.1.min.js";
	...
	protected $data_tables_ajax_url	='';
	protected $data_tables_ajax_setting	='';
	...
	public function setDataTablesAjaxUrl($url) {
		$this->data_tables_ajax_url = $url;
	}
	
	public function setDataTablesAjaxSetting($setting) {
		$this->data_tables_ajax_setting = $setting;
	}
    ...
    protected function _load_date_format()
    {
        ...
        list($js_day, $js_month, $js_year) = array('d','m','Y');
    ...
    protected function get_columns()
    {
        if($this->columns_checked === false)
        {
            ...
            foreach($this->columns as $col_num => $column)
            {
                    ...
                    if(!empty($this->relation))
                    {
                        ...
                        foreach($this->relation as $relation)
                        {
                            if( $relation[2] == $column )
                            {
                                ...
                                {
                                    $this->display_as[$new_column] = ucfirst(str_replace('_',' ',$column)) . 'uuu';
                                }
				...
				$this->columns[$col_num]->field_name_original = $field_name_original;
