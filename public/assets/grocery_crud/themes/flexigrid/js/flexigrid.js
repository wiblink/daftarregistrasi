	...
	$('.filtering_form').submit(function(){
		...
		$(this).ajaxSubmit({
			 ...
			 beforeSend: function(){
				 ...
				 $spinner = $('<div class="spinner-container bg-warning rounded d-flex align-items-center justify-content-center bg-opacity-75 flexgrid-loader-icon" style="width: 60px; height:60px; position:absolute; left:calc(50% - 30px); top:calc(50% - 30px);"><span class="spinner-border"></span></div>');
				 $spinner.appendTo($('#ajax_list'));
			 },
			 complete: function(){
 ...
 */
	...
	$('.print-anchor').click(function(){
		...
		$('.flexigrid').find('.hidden-operations').html(form_on_demand);

		var _this_button = $(this);

		$('.flexigrid').find('#print_form').ajaxSubmit({
			beforeSend: function(){
